...
function buildComparisonsDe(count: number, category: BeverageCategory, fmt: Fmt): FunComparison[] {
  if (category === 'beer') {
    return [
      ...
      {
        emoji: '📦',
        number: fmt(Math.floor(count / 14)),
        label: 'Kisten (20er)',
        highlight: false,
        matched: count >= 20,
      },
      {
        emoji: '🎪',
        number: `${fmt(Math.floor(count / 5000))}x`,
      ...
      {
        emoji: '🫗',
        number: fmt(Math.floor(count * 500)),
  ...
  if (category === 'whisky') {
    // Drams assume 70cl bottles (≈17 × 4cl). 7 of 19 entries are 50cl —
    // simplification, not a per-bottle calculation.
    return [
      {
        emoji: '🥃',
  ...
  if (category === 'coffee') {
    return [
      ...
      {
        emoji: '☕',
        number: fmt(Math.floor(count / 1095)),
  ...
  return [
    ...
    {
      emoji: '🏕️',
      number: fmt(Math.floor(count / 10)),
      label: 'Yoga-Retreats',
      highlight: false,
      matched: count >= 50,
    },
    ...
    {
      emoji: '🫐',
      number: fmt(Math.floor(count * 15)),
...
function buildComparisonsEn(count: number, category: BeverageCategory, fmt: Fmt): FunComparison[] {
  if (category === 'beer') {
    return [
      ...
      {
        emoji: '📦',
        number: fmt(Math.floor(count / 24)),
        label: 'cases (24-pack)',
        highlight: false,
        matched: count >= 24,
      },
